...
// // Парсер CPV, региона, страны, заказчика и исполнителя, ИСПРАВЛЕНО noticeId
...
//       return { cpvCodes, nutsCode, country, customer, provider, noticeId };
//     }

//     // --- ИЩЕМ NoticePublicationID ---
//     // ext:UBLExtensions > ext:UBLExtension > ext:ExtensionContent > efext:EformsExtension > efac:Publication > efbc:NoticePublicationID
//     const eformsExt = contractNotice?.["ext:UBLExtensions"]?.["ext:UBLExtension"]?.["ext:ExtensionContent"]?.["efext:EformsExtension"];
//     const publication = eformsExt?.["efac:Publication"];
//     if (publication?.["efbc:NoticePublicationID"]) {
//       noticeId = extractXmlText(publication["efbc:NoticePublicationID"]);
//     }

//     // Fallback: если не нашли, пробуем cbc:ID
//     if (!noticeId && contractNotice["cbc:ID"]) {
//       noticeId = extractXmlText(contractNotice["cbc:ID"]);
//     }
